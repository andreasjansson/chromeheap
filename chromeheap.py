def stats():
    client = RemoteInspectorClient(show_socket_messages=False)
    snapshot = client.HeapSnapshot(include_summary=True)
    client.Stop()
...
class _V8HeapSnapshotParser(object):
  ...
  def ParseSnapshotData(raw_data):
    ...
    while index < len(node_list):
      ...
      node_children_count = node_list[index + 4]
      index += 5
...
class RemoteInspectorClient(object):
  ...
  def HeapSnapshot(self, include_summary=False):
    ...
    if False and self._IsWebkitVersionNotOlderThan(537, 27):
    ...
    def HandleReply(reply_dict):
      ...
      # naively handle race condition where Stop() has already been called
      if self._remote_inspector_thread is None:
          return None
